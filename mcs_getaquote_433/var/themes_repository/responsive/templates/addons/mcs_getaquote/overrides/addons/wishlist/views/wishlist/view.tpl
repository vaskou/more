            ...
            <form class="cm-ajax cm-ajax-full-render" action="{""|fn_url}" method="post" name="vendor_form_{$vid}" enctype="multipart/form-data">
                ...
                <div class="buttons-container ty-wish-list__buttons">
                    ...
                    {if $show_getaquote}
                        ...
                        {else}
                            <a href="{if $runtime.controller == "auth" && $runtime.mode == "login_form"}{$config.current_url|fn_url}{else}{"auth.login_form?return_url=`$config.current_url`"|fn_url}{/if}" {if $settings.General.secure_auth != "Y"} data-ca-target-id="login_block{$block.snapping_id}" class="cm-dialog-opener cm-dialog-auto-size ty-btn__secondary account ty-btn"{else}class="account"{/if} rel="nofollow">{__("get_a_quote")}</a>
                        {/if}
                    {/if}
                </div>
            </form>
