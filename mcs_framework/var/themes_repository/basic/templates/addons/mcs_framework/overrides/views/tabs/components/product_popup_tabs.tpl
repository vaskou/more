    ...
    {foreach from=$tabs item="tab" key="tab_id"}
        {if $tab.show_in_popup == "Y" && $tab.status == "A"}
            ...
            {if $smarty.capture.$tab_content_capture|trim}
                <li><i class="{if $tab.icon!=''}{$tab.icon}{else}icon-popup{/if}"></i><a id="{$tab.html_id}" class="cm-dialog-opener" data-ca-target-id="content_block_popup_{$tab_id}" rel="nofollow">{$tab.name}</a></li>
